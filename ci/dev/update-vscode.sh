...
main() {
  ...
  echo -e "Great! We'll update to $VSCODE_VERSION_TO_UPDATE\n"

  # Run subtree update command
  ...
  echo "Done updating vscode to $VSCODE_VERSION_TO_UPDATE"
  echo -e "Note: there are probably merge conflicts. Please make sure to address these.\n"
  ...
  gh pr create --base master --title "chore(vscode): update to version $VSCODE_VERSION_TO_UPDATE" --body "This PR updates `/lib/vscode` to version: $VSCODE_VERSION_TO_UPDATE" --reviewer @cdr/code-server-reviewers --repo cdr/code-server --draft
}
